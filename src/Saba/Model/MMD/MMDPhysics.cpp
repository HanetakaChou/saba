...
static inline void internal_import_ragdoll(mcrt_vector<mmd_pmx_rigid_body_t> const &in_mmd_rigid_bodies, mcrt_vector<mmd_pmx_constraint_t> const &in_mmd_constraints, uint32_t const *const in_model_node_to_animation_skeleton_joint_map, DirectX::XMFLOAT4X4 const *const in_animation_pose_model_space, mcrt_vector<brx_motion_ragdoll_rigid_body> &out_ragdoll_rigid_bodies, mcrt_vector<brx_motion_ragdoll_constraint> &out_ragdoll_constraints, mcrt_vector<uint32_t> &out_ragdoll_skeleton_joint_parent_indices, mcrt_vector<brx_motion_ragdoll_direct_mapping> &out_animation_to_ragdoll_mapping, mcrt_vector<brx_motion_ragdoll_direct_mapping> &out_ragdoll_to_animation_mapping);
...
namespace saba
{
	...
	void MMDPhysics::InitRagdoll(mcrt_vector<mmd_pmx_rigid_body_t> const &in_mmd_rigid_bodies, mcrt_vector<mmd_pmx_constraint_t> const &in_mmd_joints, uint32_t const *const in_model_node_to_animation_skeleton_joint_map, glm::mat4 const *const in_animation_pose_model_space)
	{
		mcrt_vector<uint32_t> ragdoll_skeleton_joint_parent_indices;
		mcrt_vector<brx_motion_ragdoll_rigid_body> ragdoll_rigid_bodies;
		mcrt_vector<brx_motion_ragdoll_constraint> ragdoll_constraints;
		internal_import_ragdoll(in_mmd_rigid_bodies, in_mmd_joints, in_model_node_to_animation_skeleton_joint_map, reinterpret_cast<DirectX::XMFLOAT4X4 const *>(in_animation_pose_model_space), ragdoll_rigid_bodies, ragdoll_constraints, ragdoll_skeleton_joint_parent_indices, m_animation_to_ragdoll_mapping, m_ragdoll_to_animation_mapping);

		assert(m_jph_physics_rigid_bodies.empty());
		for (brx_motion_ragdoll_rigid_body const &ragdoll_rigid_body : ragdoll_rigid_bodies)
		{
			brx_physics_rigid_body *jph_physics_rigid_body = jph_physics_create_rigid_body(m_jph_physics_context, m_jph_physics_world, ragdoll_rigid_body.m_model_space_transform.m_rotation, ragdoll_rigid_body.m_model_space_transform.m_translation, ragdoll_rigid_body.m_shape_type, ragdoll_rigid_body.m_shape_size, ragdoll_rigid_body.m_motion_type, ragdoll_rigid_body.m_collision_filter_group, ragdoll_rigid_body.m_collision_filter_mask, ragdoll_rigid_body.m_mass, ragdoll_rigid_body.m_linear_damping, ragdoll_rigid_body.m_angular_damping, ragdoll_rigid_body.m_friction, ragdoll_rigid_body.m_restitution);
		...
		for (brx_motion_ragdoll_constraint const &ragdoll_constraint : ragdoll_constraints)
		{
			brx_physics_constraint *jph_physics_constraint = jph_physics_create_constraint(m_jph_physics_context, m_jph_physics_world, m_jph_physics_rigid_bodies[ragdoll_constraint.m_rigid_body_a_index], m_jph_physics_rigid_bodies[ragdoll_constraint.m_rigid_body_b_index], ragdoll_constraint.m_constraint_type, ragdoll_constraint.m_pivot, ragdoll_constraint.m_twist_axis, ragdoll_constraint.m_plane_axis, ragdoll_constraint.m_normal_axis, ragdoll_constraint.m_twist_limit, ragdoll_constraint.m_plane_limit, ragdoll_constraint.m_normal_limit);
	...
	void MMDPhysics::AnimationToRagdoll(glm::mat4x4 const *const in_animation_skeleton_model_space)
	{
		for (brx_motion_ragdoll_direct_mapping const &ragdoll_direct_mapping : m_animation_to_ragdoll_mapping)
		{
			glm::mat4 animation_transform = in_animation_skeleton_model_space[ragdoll_direct_mapping.m_joint_index_a];
	...
	void MMDPhysics::RagdollToAnimation(glm::mat4x4 *const out_animation_skeleton_model_space)
	{
		for (brx_motion_ragdoll_direct_mapping const &ragdoll_direct_mapping : m_ragdoll_to_animation_mapping)
		{
			float brx_rotation[4];
			...
			out_animation_skeleton_model_space[ragdoll_direct_mapping.m_joint_index_b] = animation_transform;
...
static inline void internal_import_ragdoll(mcrt_vector<mmd_pmx_rigid_body_t> const &in_mmd_rigid_bodies, mcrt_vector<mmd_pmx_constraint_t> const &in_mmd_constraints, uint32_t const *const in_model_node_to_animation_skeleton_joint_map, DirectX::XMFLOAT4X4 const *const in_animation_pose_model_space, mcrt_vector<brx_motion_ragdoll_rigid_body> &out_ragdoll_rigid_bodies, mcrt_vector<brx_motion_ragdoll_constraint> &out_ragdoll_constraints, mcrt_vector<uint32_t> &out_ragdoll_skeleton_joint_parent_indices, mcrt_vector<brx_motion_ragdoll_direct_mapping> &out_animation_to_ragdoll_mapping, mcrt_vector<brx_motion_ragdoll_direct_mapping> &out_ragdoll_to_animation_mapping)
{
	...
	mcrt_vector<uint32_t> ragdoll_skeleton_joint_to_rigid_body_map(static_cast<size_t>(rigid_body_count), BRX_MOTION_UINT32_INDEX_INVALID);
	mcrt_vector<uint32_t> rigid_body_to_ragdoll_skeleton_joint_map(static_cast<size_t>(rigid_body_count), BRX_MOTION_UINT32_INDEX_INVALID);
	{
		assert(out_ragdoll_skeleton_joint_parent_indices.empty());
		out_ragdoll_skeleton_joint_parent_indices = {};

		mcrt_vector<uint32_t> rigid_body_parent_indices(static_cast<size_t>(rigid_body_count), BRX_MOTION_UINT32_INDEX_INVALID);
		for (mmd_pmx_constraint_t const &mmd_constraint : in_mmd_constraints)
		{
			...
			{
				if (BRX_MOTION_UINT32_INDEX_INVALID == rigid_body_parent_indices[child_index])
				{
		...
		for (uint32_t rigid_body_index_plus_1 = rigid_body_count; rigid_body_index_plus_1 > 0U; --rigid_body_index_plus_1)
		{
			...
			if (BRX_MOTION_UINT32_INDEX_INVALID != rigid_body_parent_index)
			{
		...
		while (!rigid_body_depth_first_search_stack.empty())
		{
			...
			if (BRX_MOTION_UINT32_INDEX_INVALID == rigid_body_parent_index)
			{
				out_ragdoll_skeleton_joint_parent_indices.push_back(BRX_MOTION_UINT32_INDEX_INVALID);
			}
			else
			{
				assert(BRX_MOTION_UINT32_INDEX_INVALID != rigid_body_to_ragdoll_skeleton_joint_map[rigid_body_parent_index]);
				out_ragdoll_skeleton_joint_parent_indices.push_back(rigid_body_to_ragdoll_skeleton_joint_map[rigid_body_parent_index]);
			}

			assert(BRX_MOTION_UINT32_INDEX_INVALID == ragdoll_skeleton_joint_to_rigid_body_map[ragdoll_skeleton_joint_current_index]);
			ragdoll_skeleton_joint_to_rigid_body_map[ragdoll_skeleton_joint_current_index] = rigid_body_current_index;

			assert(BRX_MOTION_UINT32_INDEX_INVALID == rigid_body_to_ragdoll_skeleton_joint_map[rigid_body_current_index]);
...

	{
		...
		for (uint32_t ragdoll_skeleton_joint_index = 0U; ragdoll_skeleton_joint_index < rigid_body_count; ++ragdoll_skeleton_joint_index)
		{
			...
			assert(BRX_MOTION_UINT32_INDEX_INVALID != rigid_body_index);
			...
			static_assert(0 == BRX_PHYSICS_RIGID_BODY_SHAPE_SPHERE, "");
			static_assert(1 == BRX_PHYSICS_RIGID_BODY_SHAPE_BOX, "");
			static_assert(2 == BRX_PHYSICS_RIGID_BODY_SHAPE_CAPSULE, "");
			out_ragdoll_rigid_bodies.push_back(
				brx_motion_ragdoll_rigid_body{
					{{
					...
					static_cast<BRX_PHYSICS_RIGID_BODY_SHAPE_TYPE>(mmd_rigid_body.m_shape_type),
					{
						mmd_rigid_body.m_shape_size.m_x,
						mmd_rigid_body.m_shape_size.m_y,
						mmd_rigid_body.m_shape_size.m_z,
					},
					(0 == mmd_rigid_body.m_rigid_body_type) ? BRX_PHYSICS_RIGID_BODY_MOTION_KEYFRAME : BRX_PHYSICS_RIGID_BODY_MOTION_DYNAMIC,
			...
			if (BRX_MOTION_UINT32_INDEX_INVALID != mmd_rigid_body.m_bone_index)
			{
				...
				if (0 == mmd_rigid_body.m_rigid_body_type)
				{
					...
					out_animation_to_ragdoll_mapping.push_back(
						brx_motion_ragdoll_direct_mapping{
