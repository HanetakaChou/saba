...
namespace saba
{
    ...
    void MMDIkSolver::Solve()
    {
        ...
        glm::vec3 target_position_model_space = m_ikNode->SyncLocalTransformToGlobal()[3];

        glm::mat4 end_effector_transform_local_space = m_ikTarget->GetLocalTransform();

        std::vector<glm::mat4x4> joints_local_space;
        std::vector<glm::mat4x4> joints_model_space;

        for (uint32_t chain_index_plus_1 = m_chains.size(); chain_index_plus_1 >= 1U; --chain_index_plus_1)
        {
            joints_local_space.push_back(m_chains[chain_index_plus_1 - 1U].m_node->GetLocalTransform());
            joints_model_space.push_back(m_chains[chain_index_plus_1 - 1U].m_node->SyncLocalTransformToGlobal());
        }

        DirectX::XMFLOAT3 const &in_target_position_model_space = *reinterpret_cast<DirectX::XMFLOAT3 *>(&target_position_model_space);

        DirectX::XMFLOAT4X4 const &in_end_effector_transform_local_space = *reinterpret_cast<DirectX::XMFLOAT4X4 *>(&end_effector_transform_local_space);

        DirectX::XMFLOAT4X4 *const inout_joints_local_space = reinterpret_cast<DirectX::XMFLOAT4X4 *>(&joints_local_space[0]);

        DirectX::XMFLOAT4X4 *const inout_joints_model_space = reinterpret_cast<DirectX::XMFLOAT4X4 *>(&joints_model_space[0]);

#if 0
        ik_reaching_solve(*reinterpret_cast<DirectX::XMFLOAT3 *>(&in_target_position_model_space), in_end_effector_transform_local_space, m_chains.size(), inout_joints_local_space, inout_joints_model_space);
#else
        if (1 == m_chains.size())
        {
        ...
        else if (2 == m_chains.size())
        {
            IKChain const *const hinge_joint = &m_chains[0];
            ...
            DirectX::XMFLOAT3 in_hinge_joint_axis_local_space;
            float in_cosine_max_hinge_joint_angle;
            float in_cosine_min_hinge_joint_angle;
            if (hinge_joint->m_enableAxisLimit)
            {
                float rotation_limit_x = std::max(std::abs(hinge_joint->m_limitMin.x), std::abs(hinge_joint->m_limitMax.x));
                float rotation_limit_y = std::max(std::abs(hinge_joint->m_limitMin.y), std::abs(hinge_joint->m_limitMax.y));
                float rotation_limit_z = std::max(std::abs(hinge_joint->m_limitMin.z), std::abs(hinge_joint->m_limitMax.z));

                if ((rotation_limit_x >= rotation_limit_y) && (rotation_limit_x >= rotation_limit_z))
                {
                    if (hinge_joint->m_limitMin.x >= 0.0F && hinge_joint->m_limitMax.x >= 0.0F)
                    {
                        in_hinge_joint_axis_local_space.x = -1.0F;
                        in_hinge_joint_axis_local_space.y = 0.0F;
                        in_hinge_joint_axis_local_space.z = 0.0F;

                        in_cosine_max_hinge_joint_angle = std::cos(std::max(std::abs(hinge_joint->m_limitMin.x), std::abs(hinge_joint->m_limitMax.x)));
                        in_cosine_min_hinge_joint_angle = std::cos(std::min(std::abs(hinge_joint->m_limitMin.x), std::abs(hinge_joint->m_limitMax.x)));
                    }
                    else if (hinge_joint->m_limitMin.x <= 0.0F && hinge_joint->m_limitMax.x <= 0.0F)
                    {
                        in_hinge_joint_axis_local_space.x = 1.0F;
                        in_hinge_joint_axis_local_space.y = 0.0F;
                        in_hinge_joint_axis_local_space.z = 0.0F;
                    ...
                    {
                        float rotation_limit_min = std::min(hinge_joint->m_limitMin.x, hinge_joint->m_limitMax.x);
                        float rotation_limit_max = std::max(hinge_joint->m_limitMin.x, hinge_joint->m_limitMax.x);
                        ...
                        if (std::abs(rotation_limit_max) >= std::abs(rotation_limit_min))
                        {
                            ...
                            in_cosine_max_hinge_joint_angle = std::cos(std::abs(rotation_limit_max));
                            in_cosine_min_hinge_joint_angle = 1.0F;
                        }
                        else
                        {
                            in_hinge_joint_axis_local_space.x = 1.0F;
                            ...
                            in_cosine_max_hinge_joint_angle = std::cos(std::abs(rotation_limit_min));
                            in_cosine_min_hinge_joint_angle = 1.0F;
                        }
                    }
                }
                ...
                {
                    if (hinge_joint->m_limitMin.y >= 0.0F && hinge_joint->m_limitMax.y >= 0.0F)
                    {
                        in_hinge_joint_axis_local_space.x = 0.0F;
                        in_hinge_joint_axis_local_space.y = -1.0F;
                        in_hinge_joint_axis_local_space.z = 0.0F;

                        in_cosine_max_hinge_joint_angle = std::cos(std::max(std::abs(hinge_joint->m_limitMin.y), std::abs(hinge_joint->m_limitMax.y)));
                        in_cosine_min_hinge_joint_angle = std::cos(std::min(std::abs(hinge_joint->m_limitMin.y), std::abs(hinge_joint->m_limitMax.y)));
                    }
                    else if (hinge_joint->m_limitMin.y <= 0.0F && hinge_joint->m_limitMax.y <= 0.0F)
                    {
                        in_hinge_joint_axis_local_space.x = 0.0F;
                        in_hinge_joint_axis_local_space.y = 1.0F;
                        in_hinge_joint_axis_local_space.z = 0.0F;
                    ...
                    {
                        float rotation_limit_min = std::min(hinge_joint->m_limitMin.y, hinge_joint->m_limitMax.y);
                        float rotation_limit_max = std::max(hinge_joint->m_limitMin.y, hinge_joint->m_limitMax.y);
                        assert(rotation_limit_min <= 0.0F);
                        assert(rotation_limit_max >= 0.0F);
                ...
                else
                {
                    ...
                    if (hinge_joint->m_limitMin.z >= 0.0F && hinge_joint->m_limitMax.z >= 0.0F)
                    {
                        in_hinge_joint_axis_local_space.x = 0.0F;
                        in_hinge_joint_axis_local_space.y = 0.0F;
                        in_hinge_joint_axis_local_space.z = -1.0F;

                        in_cosine_max_hinge_joint_angle = std::cos(std::max(std::abs(hinge_joint->m_limitMin.z), std::abs(hinge_joint->m_limitMax.z)));
                        in_cosine_min_hinge_joint_angle = std::cos(std::min(std::abs(hinge_joint->m_limitMin.z), std::abs(hinge_joint->m_limitMax.z)));
                    }
                    else if (hinge_joint->m_limitMin.z <= 0.0F && hinge_joint->m_limitMax.z <= 0.0F)
                    {
                        in_hinge_joint_axis_local_space.x = 0.0F;
                        in_hinge_joint_axis_local_space.y = 0.0F;
                        in_hinge_joint_axis_local_space.z = 1.0F;
                    ...
                    {
                        float rotation_limit_min = std::min(hinge_joint->m_limitMin.z, hinge_joint->m_limitMax.z);
                        float rotation_limit_max = std::max(hinge_joint->m_limitMin.z, hinge_joint->m_limitMax.z);
                        assert(rotation_limit_min <= 0.0F);
                        assert(rotation_limit_max >= 0.0F);
                ...
                if (DirectX::XMVectorGetX(DirectX::XMVector3Dot(DirectX::XMLoadFloat3(&in_hinge_joint_axis_local_space), DirectX::XMLoadFloat3(&hinge_joint_normal_local_space))) < (1.0F - 5E-2F))
                {
                    assert(false);
                    in_hinge_joint_axis_local_space = hinge_joint_normal_local_space;
                    in_cosine_max_hinge_joint_angle = -1.0F;
                    in_cosine_min_hinge_joint_angle = 1.0F;
                }
            }
            else
            ...
            ik_two_joints_solve(1.0F, in_hinge_joint_axis_local_space, in_cosine_max_hinge_joint_angle, in_cosine_min_hinge_joint_angle, 1.0F, in_target_position_model_space, in_end_effector_transform_local_space, inout_joints_local_space, inout_joints_model_space);
        }
        else if (3U == m_chains.size())
        {
            // TODO: three joints IK
            ik_ccd_solve(8U, 0.5F, in_target_position_model_space, in_end_effector_transform_local_space, m_chains.size(), inout_joints_local_space, inout_joints_model_space);
        }
        else
        {

            assert(m_chains.size() >= 4U);
            ik_ccd_solve(8U, 0.5F, in_target_position_model_space, in_end_effector_transform_local_space, m_chains.size(), inout_joints_local_space, inout_joints_model_space);
        }
#endif

        {
            ...
            {
                glm::quat updated_current_joint_local_space_rotation = glm::quat_cast(glm::mat3(joints_local_space[current_joint_chain_index]));

                IKChain *const chain = &m_chains[chainIdx];

                MMDNode *const chainNode = chain->m_node;
