...
namespace saba
{
    MMDIkSolver::MMDIkSolver(std::string const &name)
        : m_name(name), m_enable(true), m_baseAnimEnable(true)
    {
    }

    void MMDIkSolver::Solve(glm::vec3 const &in_two_joints_hinge_joint_axis_local_space, float const in_two_joints_cosine_max_hinge_joint_angle, float const in_two_joints_cosine_min_hinge_joint_angle, glm::vec3 const &in_target_position_model_space, glm::mat4 const &in_end_effector_transform_local_space, uint32_t const in_joint_count, glm::mat4x4 *inout_joints_local_space, glm::mat4x4 *inout_joints_model_space)
    {
#if 0
        ik_reaching_solve(*reinterpret_cast<DirectX::XMFLOAT3 const *>(&in_target_position_model_space), *reinterpret_cast<DirectX::XMFLOAT4X4 const *>(&in_end_effector_transform_local_space), in_joint_count, reinterpret_cast<DirectX::XMFLOAT4X4 *>(inout_joints_local_space), reinterpret_cast<DirectX::XMFLOAT4X4 *>(inout_joints_model_space));
#else
        if (1U == in_joint_count)
        {
            ik_one_joint_solve(1.0F, *reinterpret_cast<DirectX::XMFLOAT3 const *>(&in_target_position_model_space), *reinterpret_cast<DirectX::XMFLOAT4X4 const *>(&in_end_effector_transform_local_space), reinterpret_cast<DirectX::XMFLOAT4X4 *>(inout_joints_local_space), reinterpret_cast<DirectX::XMFLOAT4X4 *>(inout_joints_model_space));
        }
        else if (2U == in_joint_count)
        {
            ik_two_joints_solve(1.0F, *reinterpret_cast<DirectX::XMFLOAT3 const *>(&in_two_joints_hinge_joint_axis_local_space), in_two_joints_cosine_max_hinge_joint_angle, in_two_joints_cosine_min_hinge_joint_angle, 1.0F, *reinterpret_cast<DirectX::XMFLOAT3 const *>(&in_target_position_model_space), *reinterpret_cast<DirectX::XMFLOAT4X4 const *>(&in_end_effector_transform_local_space), reinterpret_cast<DirectX::XMFLOAT4X4 *>(inout_joints_local_space), reinterpret_cast<DirectX::XMFLOAT4X4 *>(inout_joints_model_space));
        }
        else if (3U == in_joint_count)
        {
            // TODO: three joints IK
            ik_ccd_solve(8U, 0.5F, *reinterpret_cast<DirectX::XMFLOAT3 const *>(&in_target_position_model_space), *reinterpret_cast<DirectX::XMFLOAT4X4 const *>(&in_end_effector_transform_local_space), in_joint_count, reinterpret_cast<DirectX::XMFLOAT4X4 *>(inout_joints_local_space), reinterpret_cast<DirectX::XMFLOAT4X4 *>(inout_joints_model_space));
        }
        else
        {
